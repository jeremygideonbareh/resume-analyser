...
**Status:** FINAL — APPROVED 2026-08-19 (explicit user approval via approval question: "Approve — finalize it")
**Date:** 2026-08-19
...
- [x] **Phase A — Auth-flow library rewrite**: `src/lib/auth-flow.ts` gains `signUpWithEmail`, `signInWithPassword`, `sendPasswordResetEmail` (redirectTo = origin+pathname), `updatePasswordFromRecovery`; removes all four OTP functions; `session.ts` surfaces recovery flag; tests rewritten (auth-flow.test.ts + auth-validation.test.ts). Verify: `npm test` green.
- [x] **Phase B — LoginPanel rewrite**: email+password Sign in / Create account tabs, forgot-password flow, recovery view; phone tab removed; tests rewritten. Verify: `npm test` green + dev-mode manual pass.
