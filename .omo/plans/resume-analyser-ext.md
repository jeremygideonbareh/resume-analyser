...
2. **Real sign-in via Supabase Auth (email OTP + phone OTP)** — we provision a brand-new free-tier Supabase project from your Management API token (you chose "real auth, keep privacy-first" + "provision new project"), wire `@supabase/supabase-js`, and replace the in-memory demo login with real auth flows: enter email (or phone) → Supabase sends a 6-digit code → enter it → signed in. Sessions persist via Supabase's standard auth token (your choice: kept on-device, user-removable by logging out or clearing site data). **Privacy-first stays:** no resume uploads, no analysis storage, no analytics, no user data beyond the sign-in identifier — the analyser itself remains 100% local in the browser.
...
**What it will NOT do:** No resume/analysis storage, no user profile database, no analytics, no cookies for tracking, no WebGL/particle effects, no changes to the analyser core, no serverless functions (all auth is Supabase-managed; the site stays a static Vite SPA). The only new runtime dependency is `@supabase/supabase-js` (the auth client) — any `framer-motion` import from Componentry is rewritten to `motion/react`.

**Effort:** 9 todos across 3 waves (Componentry integration, Supabase auth, final verification F1–F4). **Risk:** Medium — new external dependency on the Supabase Management API for provisioning (fallbacks: `supabase` CLI or manual console steps documented), and the phone-OTP SMS-provider caveat above.
...
5. Session state via `supabase.auth.onAuthStateChange` (the official pattern), persisted with Supabase's default token storage; privacy copy updated to stay truthful ("no uploads, no analysis storage; sign-in keeps a session token on your device, removable any time").
...
3. **Privacy-first guardrails** — no resume/analysis storage, no user data beyond the sign-in identifier, no analytics; the analyser stays 100% local; privacy copy updated to remain truthful about the on-device session token.
4. **Docs** — Componentry provenance + Supabase auth setup/limits in HANDOFF.md (fulfilling the original provenance promise), README update.
5. **Final verification wave** — F1 plan compliance, F2 code quality review, F3 real browser QA, F4 scope fidelity; all four must APPROVE; surface results and wait for the user's explicit okay.

**Out of scope / Must-NOT-Have:**
- NO storage of resumes or analyses (no tables for user content; Supabase Auth's own user table only), NO analytics, NO tracking cookies, NO profile/payment features.
...
- NO new runtime dependencies except `@supabase/supabase-js` (motion is already present; `framer-motion` imports, if any, MUST be adapted to `motion/react` — never add framer-motion as a dependency).
...
- [ ] **Todo 1.3 — ScrollVelocity strip in SkillsMarquee + FlippingWordSwap in Header CTA**
...
- Steps:
  ...
  4. Privacy copy reconciliation: Footer privacy line + README privacy bullet updated to remain truthful: "No uploads. No analysis storage. No cookies. Sign-in keeps a session token on your device — removable any time by logging out or clearing site data." (Verify against current text and adjust minimally; keep the existing tone.)
...
### Wave 3 — Final verification wave

- [ ] **Todo 3.1 — F1–F4 final verification (same protocol as original build)**
...
- Steps:
  ...
  4. F4 scope fidelity: grep confirms no resume/analysis storage tables, no analytics, no serverless functions, no WebGL/particle components, no analyser-core changes, `service_role` never in client code, privacy copy truthful, all 7 sections kinetic + real auth delivered.
...
- **F4 Scope fidelity:** confirm nothing from Scope OUT was built (no resume/analysis storage, no analytics, no serverless, no WebGL, no core changes, no `service_role` in client, no extra deps, privacy copy truthful) and nothing in Scope was dropped (all 7 sections kinetic, real Supabase auth delivered, docs updated).
...
- Suggested sequence: `feat:` ×6 (1.1–1.4, 2.2, 2.3), `chore:` ×2 (2.1 provision, 3.1 evidence), `docs:` (2.4).
...
5. **Privacy-first intact:** no resume/analysis storage, no analytics, no user data beyond the sign-in identifier; analyser stays 100% local; privacy copy truthful about the on-device session token; `service_role` never in client code; no secrets committed (grep-verified).
6. No `framer-motion` dependency added (imports rewritten to `motion/react`); no WebGL/particle components; analyser core untouched; only `@supabase/supabase-js` added to deps.
7. All tests green: 57 existing + new auth-validation/auth-flow/session/supabase-client/LoginPanel tests; typecheck 0, lint 0, build 0.
8. Docs: HANDOFF Componentry provenance + Supabase setup/limits/SMS caveat/Vercel env note; README updated with the real-auth feature and updated privacy wording.
9. Final verification wave F1–F4 all APPROVE and the user gives explicit okay before "done" is claimed.
