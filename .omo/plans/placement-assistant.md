...
- **T6.2 — Vercel deploy.** — **DONE 2026-08-20** — LIVE at https://resume-lab-one.vercel.app (project cloudlyconfusing-4947s-projects/resume-lab). Env vars set: LLM_API_KEY (Groq), LLM_BASE_URL, LLM_MODEL (llama-3.3-70b-versatile), VITE_ENABLE_LLM=true, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY. Deploy fixes: base path conditional on VERCEL env (5c2da5c), api imports .js extensions (44a5475), api handlers rewritten to Vercel Node.js (req,res) signature + @vercel/node devDep (30a3a99) — fixed "request.text is not a function" runtime crash. Verified: site 200, assets 200, /api/chat 401 (no auth), /api/grammar 400 (empty), /api/analyze 400 (empty). Evidence `.omo/evidence/pl-6-deploy.log`.
- **T6.3 — Live smoke (all 4 modules).** — **DONE 2026-08-20** — deployed URL https://resume-lab-one.vercel.app; Supabase URL config updated (SITE_URL = Vercel, allow-list includes Vercel + GitHub Pages + localhost); API endpoints verified (chat 401 no-auth, grammar 400 empty, analyze 400 empty); sign-in verified (3 users in Supabase Auth, VITE_SUPABASE vars inlined in bundle); full browser journey completed by user (sign up → profile → chat eligibility → analyse + grammar → dashboard → logout/login persistence), 0 console errors. Evidence `.omo/evidence/pl-6-live.log`.
...
- **F3 — Live production QA:** **APPROVE 2026-08-20** — deployed Vercel URL https://resume-lab-one.vercel.app; full journey verified live (sign up → profile → chat eligibility → analyse + grammar + improvements → dashboard readiness/applications; logout/login/reload persistence; marquee/hero unchanged; guest gate intact; 0 console errors); screenshots + `.omo/evidence/pl-6-live.log` + `pl-final-f3-manual-qa.log`.
...
- [x] Execution: user runs `/start-work` (or equivalent) — **DONE 2026-08-20** — executor worked through TODOs in order; T1.1–T6.5 complete, deployed live at https://resume-lab-one.vercel.app, **F1/F2/F3/F4 ALL APPROVE**.