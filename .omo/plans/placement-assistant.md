...
- **T6.3 — Live smoke (all 4 modules).** — **PARTIAL 2026-08-20** — deployed URL https://resume-lab-one.vercel.app live; API endpoints verified (chat 401 no-auth, grammar 400 empty, analyze 400 empty); full browser journey (sign up → profile → chat eligibility → analyse + grammar → dashboard → logout/login persistence) needs manual browser verification against the live URL. Evidence `.omo/evidence/pl-6-deploy.log`.
